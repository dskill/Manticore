...
public class tk2dSpriteCollectionBuilder
{
	...
	static void RefreshExistingAssets(tk2dSpriteCollectionData spriteCollectionData)
	{
        ...
        foreach (tk2dSprite spr in sprs)
        {
			...
			if (spr.collection == spriteCollectionData)
			{
		...
		tk2dSlicedSprite[] slicedSprites = Resources.FindObjectsOfTypeAll(typeof(tk2dSlicedSprite)) as tk2dSlicedSprite[];
		foreach (var spr in slicedSprites)
		{
			if (IsPrefab(spr))
				continue;
		...
		foreach (var spr in batchedSprs)
		{
			...
			if (spr.spriteCollection == spriteCollectionData)
			{
				spr.Build();
			}
		}
		tk2dTextMesh[] textMeshes = Resources.FindObjectsOfTypeAll(typeof(tk2dTextMesh)) as tk2dTextMesh[];
